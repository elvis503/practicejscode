const errorMsg = '<p><em>You do not have any list items yet. Try adding one with the form above.</em></p>';
...
function sessionTemplate (sessions) {
    ...
    if (sessions < 1) return errorMsg;
    return '' +
      sessions.map(applyTemplate).join('') + '';
    
};

export {sessionTemplate, errorMsg};
